...
func initialState(maze Maze) (start math.Vector2[int], allKeys Keys) {
...
func findMinSteps(
	maze Maze, minSteps *int, steps int, position math.Vector2[int], keys, allKeys Keys,
	...
	pk := ProcessedKey{position: position, collectedKeys: keys}
				...
				if tile != passageTile {
					if tile.Key() {
						if nk, added := keys.Add(tile); added {
							findMinSteps(maze, minSteps, steps, pos, nk, allKeys, processed)
							continue
						}
...
func process(maze Maze) int {
	start, allKeys := initialState(maze)
	maze[start.Y][start.X] = passageTile
	minSteps := stdmath.MaxInt
	findMinSteps(maze, &minSteps, 0, start, Keys(0), allKeys, map[ProcessedKey]int{})
